...
for the mine-tick-gravity scanner-separated domain.
...
def generate_pddl_problem(
    ...
    # ------------------------------------------------------------------
    obj_lines = []
    obj_lines.append(f"    {' '.join(real_cells)} - real-cell")
    ...
    init_lines.append(f"    (border-cell {scan_end})")
    init_lines.append(f"    (empty {scan_end})")
    ...
    # :goal
    # ------------------------------------------------------------------
    # Simple default: eventually get a gem and finish the scan tick.
    goal_lines = [
        "    (got-gem)",
        "    (scan-complete)",
        "    (not (crushed))",
...
def main():
    ...
    if args.problem_name == "":
        args.problem_name = (
            level_input.rsplit(".", 1)[0] if level_input.endswith(".txt") else "level"
        )
